...
INTRO_NAME_DURATION = 6.0
INTRO_NAME_CARDS = {
    "metan": ("CharacterNameMetan", "紫ノ宮 澪", 260, 865),
    "zunda": ("CharacterNameZunda", "麦野 透", 1735, 865),
}
...
def _build_ass(events: list[SubEvent]) -> str:
    ...
    for speaker, (style, _, _, _) in INTRO_NAME_CARDS.items():
        color = SPEAKER_MAP[speaker][2]
        styles.append(
            f"Style: {style},{SUB_FONT},44,&H00FFFFFF,&H00FFFFFF,"
            f"{_ass_color(color)},&H60000000,-1,0,0,0,100,100,0,0,"
            "3,14,2,5,0,0,0,1"
        )
    dialogues = []
    if events:
        intro_end = min(INTRO_NAME_DURATION, max(event.end for event in events))
        for _, (style, label, x, y) in INTRO_NAME_CARDS.items():
            dialogues.append(
                f"Dialogue: 1,{_ass_time(0)},{_ass_time(intro_end)},"
                f"{style},,0,0,0,,"
                f"{{\\an5\\pos({x},{y})\\fad(180,400)}}{label}"
            )
    for ev in events:
