...
- **3層コピーは NotebookLM chat で生成**（`_THUMB_COPY_QUESTION` → `_generate_thumb_copy`）: JSON で`top`（主役ワード、全角7字以内・一般語）、`mid`（補足、全角9字以内＝縮小しても読める）、`bottom`（ベネフィット、全角8字以内・数字可）、`highlight`（bottom 内の強調1語）を1回で生成。失敗・不正時は top→カテゴリラベル（論文解説/AIニュース/AI開発/ビジネス、既定 AI要約）、bottom→動画タイトルにフォールバックする。タイトルが長い場合は `thumbnail.truncate_at_phrase` で語中を割らずに丸める（日本語は budoux の文節境界、英語は単語境界。素朴な文字数切りは「AIが勝手に3組織をハ」のように意味が壊れる）。合成前に `sanitize_public_text` で個人情報を除去する。
