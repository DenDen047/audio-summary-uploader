...
def test_video_intro_identifies_both_characters_for_six_seconds() -> None:
    ass = _build_ass([SubEvent(0.0, 8.0, "metan", "はじめましょう。")])

    assert (
        "Dialogue: 1,0:00:00.00,0:00:06.00,CharacterNameMetan"
        in ass
    )
    assert "紫ノ宮 澪" in ass
    assert (
        "Dialogue: 1,0:00:00.00,0:00:06.00,CharacterNameZunda"
        in ass
    )
    assert "麦野 透" in ass
